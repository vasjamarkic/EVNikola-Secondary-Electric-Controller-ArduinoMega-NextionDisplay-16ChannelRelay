...
const int TurnRight = 25;            // relay 2 - 2x right turning lights (via flashing relay 103 131) (both turns on via SWITCH3)
const int ParkLight =  18;           // relay 3 - 2x front parklights, 2x back parklights + registar tables lights
const int DayLight = 27;             // relay 4 - 2x front daylights H4
const int LongLight = 14;            // relay 5 - 2x front longlights H4
const int StopLight = 29;            // relay 6 - 2x back stoplights
const int Horn = 11;                 // relay 7 - to horn acuator
const int DefrostWindow = 31;        // relay 8 - to rear window defrost
const int BackwardLight = 12;        // relay 9 - to rear backlight(s)
const int WiperRear = 30;            // relay 10 - motorpump rear AND motor wiper rear
const int FogLight = 15;             // relay 11 - 2x front foglights
const int MotorLight = 28;           // relay 12 - lights when cover motor is up ...
const int WindowWater = 19;          // relay 13 - motorpump front
const int Wiper1Alternate = 26;      // relay 14 - via auto wiper intermittent relay to motor wiper front, timer
const int Wiper1 = 23;               // relay 15 - wire to auto wiper intermittent relay
...
bool ParkLight_Switch_ON = 0;
bool DayLight_Switch_ON = 0;
...
bool MotorCover_Switch_ON = true;
...
bool ChargeMode_ON = true;       // if we plug the AC charging
...
// INPUTS:
  ...
  pinMode(MotorCover_Switch, INPUT);
...
// read the state of the pushbutton value:
  ...
  MotorCover_Switch_ON = digitalRead(MotorCover_Switch);
  ...
  // flashing lights LEFT - relay 1 - wh. sw. 1 Right Side
  ...
  // flashing lights RIGHT - relay 2 - wh. sw. 1 Right Side
  ...
  // Emergence light on armature switch - relay 1 and 2 - Sw2 - blinking!!!
  if (EmergenceLight_Switch_ON == LOW) {
    digitalWrite(TurnLeft, HIGH);
    digitalWrite(TurnRight, HIGH);
    j5.setValue(100);
  } else {
...
// V TAKI KONFIGURACIJI VSI SMERNIKI UTRIPAJO!!! PROBLEM, KO VEČKRAT KLIČEMO DIGITALWRITE ... HM??
  
// when contact turns park lights on - relay 3 - Sw1
  if (ParkLight_Switch_ON == LOW) {
    digitalWrite(ParkLight, HIGH);
  } else {
    digitalWrite(ParkLight, LOW);
  }  
// switch on armature for day lights - relay 4 - Sw1
  if (DayLight_Switch_ON == LOW) {
    j0.setValue(0);
...
// switch on long lights on steering wheel - relay 5 - WH.sw1 Left Side
...
// micro switch on foot brake - relay 6 - uSw4
  if (StopLight_switch_ON == LOW) {
    digitalWrite(StopLight, HIGH);
  } else {
    digitalWrite(StopLight, LOW);
  }
// button on steering wheel - relay 7 - Wh.sw.1 Right Side (35)
...
// defrost switch on armature board - relay 8 - sw4
...
// two-pole switch (for changing drive direction/connected with Emsiso) - relay 9 - 2pSw
...
// Activation of rear wiper and watering pump - Relay 10 - Wh.Sw.2
...
// Fog lights on armature switch - relay 11 - sw2
...
// micro switch motor lid open - relay 12 - uSw1
  if (MotorCover_Switch_ON == HIGH) {
    digitalWrite(MotorLight, LOW);
    //j?.setValue(100); 
    //display on nextion MOTOR OPEN <----- create ICON!!!! on nextion
  } else {
...
// Relay 14
 if (Wiper1Alternate_Switch_ON == LOW)
  {
